...
$TEXT_DUMPER_STRING_COUNT = TEXT_DUMPER_STRING_COUNT;
		...
		if ($this->mysql_version > 40101 && CHARSET1 != 'auto') {
			mysql_query("SET NAMES '" . CHARSET1 . "'") or trigger_error ($TEXT_DUMPER_CHARSET_ERROR . "<br />" . mysql_error(), E_USER_ERROR);
...
$TEXT_DUMPER_NO = TEXT_DUMPER_NO;
		...
		if ($this->mysql_version > 40101 && (CHARSET1 != 'auto' || $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
			    ...
			    if (preg_match("/;$/", $str)) {
					...
					if (empty($insert)) {
						...
						elseif (preg_match("/CREATE table/i", $sql)){
							...
							elseif(CHARSET1 == 'auto'){ // Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
