		...
		if (isset($filelayout['v_manufacturers_name'])){
			if ($row['v_manufacturers_id'] != ''){
				$sql2 = "SELECT manufacturers_name
					...
					WHERE languages_id = " . $epdlanguage_id . " and	manufacturers_id = " . $row['v_manufacturers_id']
					;
				$result2 = tep_db_query($sql2);
				$row2 =  tep_db_fetch_array($result2);
...
//     }else{
//          echo EASY_ERROR_6 .  '<span class=smallText><a href="' . tep_href_link(FILENAME_EASYPOPULATE) . '">' . EASY_ERROR_6a . '</a></span><br>';
//	 die();
//
//      }
	...
	// move the file to where we can work with it
	$csv_upload = &tep_try_upload('usrfl', DIR_FS_DOCUMENT_ROOT . $tempdir);
...
<b><?php echo EASY_LABEL_LIMIT_CAT;?></b>
					...
					echo  tep_draw_pull_down_menu('limit_cat', tep_get_category_tree(), 0);
...
<b><?php echo EASY_LABEL_LIMIT_MAN;?></b>
				...
				$manufacturers_array[] = array('id' => $manufacturers['manufacturers_id'],
				 ...
				 echo tep_draw_pull_down_menu('limit_man', $manufacturers_array, $_POST['limit_man']);
...
// end of EP for extra field code ======= DEVSOFTVN================       
    ...
    case 'category':
      if ($limit_cat == '0'){
        ...
        /**************/
        $str_query1 = tep_db_query("select parent_id from categories where categories_id = '".$_POST['limit_cat']."'");
...
//     $categories_range=substr_replace($categories_range, ')and ', -4);
	...
	case 'attrib':
		...
		$catfeild=tep_get_category_treea($_POST['limit_cat']);
	          $categories_range .= "( ";
...
//============ EP for product extra fields Contrib by minhmt DEVSOFTVN off============
	...
	while ($row){
		...
		foreach ($langcode as $key => $lang){
			...
			$row['v_products_info_' . $lang['id']] 	= $row2['products_info'];
		...
		if ($row['v_manufacturers_id'] != ''){
			$sql2 = "SELECT manufacturers_name
				FROM ".TABLE_MANUFACTURERS_INFO."
				WHERE
				manufacturers_id = " . $row['v_manufacturers_id'] . " and languages_id = " . $epdlanguage_id;
				;
			$result2 = tep_db_query($sql2);
			$row2 =  tep_db_fetch_array($result2);
			$row['v_manufacturers_name'] = $row2['manufacturers_name'];
	...
	if ( isset($v_manufacturers_name) && $v_manufacturers_name != '' ){
		$sql = "SELECT man.manufacturers_id
			...
			WHERE
				man.manufacturers_name = '" . $v_manufacturers_name . "' and languages_id = '" . $epdlanguage_id . "' ";
		$result = tep_db_query($sql);
		$row =  tep_db_fetch_array($result);
		if ( $row != '' ){
			foreach( $row as $item ){
			...
			if (!is_numeric($max_mfg_id) ){
				...
				$sql1 = "INSERT INTO ".TABLE_MANUFACTURERS."(
				manufacturers_id,
				manufacturers_image,
				date_added,
				last_modified
					...
					if (tep_db_num_rows($result) == 0) {
						...
						$sql =
							"UPDATE ".TABLE_PRODUCTS_DESCRIPTION." SET
								...
								products_info='".$v_products_info[$key] . "',
