    ...
    $customers_id = tep_db_insert_id();
      ...
      tep_db_query("delete from " . TABLE_CUSTOMERS_TO_EXTRA_FIELDS . " where customers_id=" . (int)$customers_id);
    	  ...
    	  while($extra_fields = tep_db_fetch_array($extra_fields_query))
            ...
            $sql_data_array1 = array('customers_id' => (int)$customers_id,
					  ...
					  $sql_data_array1 = array('customers_id' => (int)$customers_id,
						...
						for($i = 1; $i <= $_POST['fields_' . $extra_fields['fields_id'] . '_total']; $i++)
						{
							if(isset($_POST['fields_' . $extra_fields['fields_id'] . '_' . $i]))
							{
							  if($is_add)
							  {
                  $sql_data_array1['value'] .= "\n";
								}
								else
								{
                  $is_add = true;
								}
              	$sql_data_array1['value'] .= $_POST['fields_' . $extra_fields['fields_id'] . '_' . $i];
							}
						}
					}

					tep_db_perform(TABLE_CUSTOMERS_TO_EXTRA_FIELDS, $sql_data_array1);
      	}
