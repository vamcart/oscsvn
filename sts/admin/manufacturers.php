<?php
/*
  $Id: manufacturers.php,v 1.2 2003/09/24 15:18:15 wilt Exp $
...
// BOF manufacturers meta tags	// Return the manufacturers meta title in the needed language	// TABLES: manufacturers_info
	  function tep_get_manufacturers_meta_title($manufacturer_id, $language_id) {
	...
	// Return the manufacturers meta keywords in the needed language	// TABLES: manufacturers_info
	  function tep_get_manufacturers_meta_keywords($manufacturer_id, $language_id) {
	    $manufacturer_query = tep_db_query("select manufacturers_meta_keywords from " . TABLE_MANUFACTURERS_INFO . " where manufacturers_id = '" . (int)$manufacturer_id . "' and languages_id = '" . (int)$language_id . "'");
	    $manufacturer = tep_db_fetch_array($manufacturer_query);
	    return $manufacturer['manufacturers_meta_keywords'];
	  }

	// Return the manufacturers meta description in the needed language	// TABLES: manufacturers_info
	  function tep_get_manufacturers_meta_description($manufacturer_id, $language_id) {
...
// BOF manufacturers descriptions + meta tags
					$sql_data_array = array_merge($sql_data_array, array('manufacturers_name' => tep_db_prepare_input($manufacturers_name_array[$language_id]), 'manufacturers_description' => tep_db_prepare_input($manufacturers_description_array[$language_id]),'manufacturers_meta_title' => tep_db_prepare_input($manufacturers_meta_title_array[$language_id]),'manufacturers_meta_keywords' => tep_db_prepare_input($manufacturers_meta_keywords_array[$language_id]),'manufacturers_meta_description' => tep_db_prepare_input($manufacturers_meta_description_array[$language_id]),));
// EOF manufacturers descriptions + meta tags
...
//$manufacturers_query_raw = "select manufacturers_id, manufacturers_name, manufacturers_image, date_added, last_modified from " . TABLE_MANUFACTURERS . " order by manufacturers_name";
  $manufacturers_query_raw = "select m.manufacturers_id, mi.manufacturers_name, m.manufacturers_image, m.date_added, m.last_modified from " . TABLE_MANUFACTURERS . " m left join " . TABLE_MANUFACTURERS_INFO . " mi on (m.manufacturers_id=mi.manufacturers_id and mi.languages_id ='" . (int)$languages_id . "') order by manufacturers_name";
// BOF manufacturers descriptions
  $manufacturers_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $manufacturers_query_raw, $manufacturers_query_numrows);
...
?>
              <tr>
                <td colspan="2"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $manufacturers_split->display_count($manufacturers_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_MANUFACTURERS); ?></td>
                    <td class="smallText" align="right"><?php echo $manufacturers_split->display_links($manufacturers_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
                </table></td>
              </tr>
<?php
...
// BOF manufacturers meta tags
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_title[' . $languages[$i]['id'] . ']');
      }
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_keywords[' . $languages[$i]['id'] . ']');
      }
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_description[' . $languages[$i]['id'] . ']');
      }
...
// BOF manufacturers meta tags
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_title[' . $languages[$i]['id'] . ']', tep_get_manufacturers_meta_title($mInfo->manufacturers_id, $languages[$i]['id']));
      }
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_TITLE . $manufacturer_inputs_string);
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_keywords[' . $languages[$i]['id'] . ']', tep_get_manufacturers_meta_keywords($mInfo->manufacturers_id, $languages[$i]['id']));
      }
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_KEYWORDS . $manufacturer_inputs_string);
      ...
      $languages = tep_get_languages();
      for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
        $manufacturer_inputs_string .= '<br>' . tep_image(DIR_WS_CATALOG_LANGUAGES . $languages[$i]['directory'] . '/images/' . $languages[$i]['image'], $languages[$i]['name']) . '&nbsp;' . tep_draw_input_field('manufacturers_meta_description[' . $languages[$i]['id'] . ']', tep_get_manufacturers_meta_description($mInfo->manufacturers_id, $languages[$i]['id']));
      }
      $contents[] = array('text' => '<br>' . TEXT_MANUFACTURERS_META_DESCRIPTION . $manufacturer_inputs_string);
