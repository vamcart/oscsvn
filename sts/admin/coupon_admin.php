...
//      $mimemessage->send($mail['customers_firstname'] . ' ' . $mail['customers_lastname'], $mail['customers_email_address'], '', $from, $subject);
  ...
  switch ($_GET['action']) {
    case 'confirmdelete':
      $delete_query=tep_db_query("update " . TABLE_COUPONS . " set coupon_active = 'N' where coupon_id='".$_GET['cid']."'");
      break;
    case 'update':
...
<?php
    ...
    $cc_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $cc_query_raw, $cc_query_numrows);
...
?>
      ...
      <tr>
       ...
       <tr>
        ...
        <td align="left"><?php echo tep_draw_input_field('coupon_products', $coupon_products); ?> <A HREF="validproducts.php" TARGET="_blank" ONCLICK="window.open('validproducts.php', 'Valid_Products', 'scrollbars=yes,resizable=yes,menubar=yes,width=600,height=600'); return false"><?php echo COUPON_VIEW; ?></A></td>
        <td align="left" class="main"><?php echo COUPON_PRODUCTS_HELP; ?></td>
      </tr>
      <tr>
        <td align="left" class="main"><?php echo COUPON_CATEGORIES; ?></td>
        <td align="left"><?php echo tep_draw_input_field('coupon_categories', $coupon_categories); ?> <A HREF="validcategories.php" TARGET="_blank" ONCLICK="window.open('validcategories.php', 'Valid_Categories', 'scrollbars=yes,resizable=yes,menubar=yes,width=600,height=600'); return false"><?php echo COUPON_VIEW; ?></A></td>
...
?>
      ...
      <tr>
        ...
        <td align="left"><?php echo '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, ''); ?>"><?php echo tep_image_button('button_cancel.gif', IMAGE_CANCEL); ?></a>
      </td>
      </tr>
      </td></table></form>
      </tr>
...
?>
          <tr>
            <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr>
                <td class="smallText">&nbsp;<?php echo $cc_split->display_count($cc_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_COUPONS); ?>&nbsp;</td>
                <td align="right" class="smallText">&nbsp;<?php echo $cc_split->display_links($cc_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?>&nbsp;</td>
              </tr>

              <tr>
                <td align="right" colspan="2" class="smallText"><?php echo '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'page=' . $_GET['page'] . '&cID=' . $cInfo->coupon_id . '&action=new') . '">' . tep_image_button('button_insert.gif', IMAGE_INSERT) . '</a>'; ?></td>
              </tr>
            </table></td>
          </tr>
        </table></td>
    ...
    default:
      ...
      if ($_GET['action'] == 'voucherdelete') {
        $contents[] = array('text'=> TEXT_CONFIRM_DELETE . '</br></br>' .
                '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'action=confirmdelete&cid=' . $_GET['cid'], 'NONSSL').'">' . tep_image_button('button_confirm.gif', 'Confirm Delete Voucher') . '</a>' .
                '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'cid=' . $cInfo->coupon_id,'NONSSL') . '">' . tep_image_button('button_cancel.gif','Cancel') . '</a>'
                );
      } else {
        $prod_details = NONE;
        ...
        $contents[] = array('text'=>COUPON_NAME . '&nbsp;::&nbsp; ' . $coupon_name['coupon_name'] . '<br>' .
                     ...
                     '<center><a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'action=email&cid=' . $cInfo->coupon_id,'NONSSL') . '">' . tep_image_button('button_email.gif','Email Voucher') . '</a>' .
                     '<a href="' . tep_href_link(FILENAME_COUPON_ADMIN, 'action=voucheredit&cid=' . $cInfo->coupon_id,'NONSSL') . '">' . tep_image_button('button_edit.gif', 'Edit Voucher') . '</a>' .
                     '<a href="'.tep_href_link(FILENAME_COUPON_ADMIN, 'action=voucherdelete&cid='.$cInfo->coupon_id,'NONSSL') . '">' . tep_image_button('button_delete.gif', 'Delete Voucher') . '</a>' .
                     '<br><a href="'.tep_href_link(FILENAME_COUPON_ADMIN, 'action=voucherreport&cid='.$cInfo->coupon_id,'NONSSL') . '">' . tep_image_button('button_report.gif', 'Voucher Report') . '</a></center>'
                     );
        }
        break;
      }
