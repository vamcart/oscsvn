...
// define our link functions
  ...
  $_GET['cID'] = (isset($_GET['cID']) ? $_GET['cID'] : '1');
...
//        }
        ...
        $link_categories_image = new upload('link_categories_image', DIR_FS_CATALOG_IMAGES);
        if (tep_not_null($link_categories_image->filename)) {
          tep_db_query("update " . TABLE_LINK_CATEGORIES . " set link_categories_image = '" . tep_db_input($link_categories_image->filename) . "' where link_categories_id = '" . (int)$link_categories_id . "'");
        }
      ...
      default:
        ...
        $links_count_query = tep_db_query("select count(*) as link_categories_count from " . TABLE_LINKS_TO_LINK_CATEGORIES . " where link_categories_id = '" . (int)$_GET['cID'] . "'");
...
<?php
    ...
    $link_categories_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $link_categories_query_raw, $link_categories_query_numrows);
...
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $link_categories_split->display_count($link_categories_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_LINK_CATEGORIES); ?></td>
                    <td class="smallText" align="right"><?php echo $link_categories_split->display_links($link_categories_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page'], tep_get_all_get_params(array('page', 'info', 'x', 'y', 'cID'))); ?></td>
                  </tr>
                  <tr>
<?php
    if (isset($_GET['search']) && tep_not_null($_GET['search'])) {
