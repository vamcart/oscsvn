<?php
/*
  $Id: webtopay.php 1803 2008-01-11 18:16:37Z hpdl $
...
// class methods
    ...
    function selection() {
      global $cart_webtopay_id;

      if (tep_session_is_registered('cart_webtopay_id')) {
        ...
        if (tep_db_num_rows($check_query) < 1) {
          ...
          tep_session_unregister('cart_webtopay_id');
        }
      }
    ...
    function confirmation() {
      ...
      if (tep_session_is_registered('cartID')) {
        ...
        if ($insert_order == true) {
          ...
          tep_session_register('cart_webtopay_id');
        }
      }
    ...
    function process_button() {
      global $customer_id, $order, $sendto, $currency, $currencies, $cart_webtopay_id, $shipping;

      $process_button_string = '';

      $process_button_string = tep_draw_hidden_field('MerchantID', MODULE_PAYMENT_WEBTOPAY_ID) .
                               tep_draw_hidden_field('OrderID', substr($cart_webtopay_id, strpos($cart_webtopay_id, '-')+1)) .
                               tep_draw_hidden_field('Lang', 'RUS') .
                               tep_draw_hidden_field('Amount', number_format($order->info['total']/(1/$currencies->get_value('USD')),2)*100) .
                               tep_draw_hidden_field('Currency', 'USD') .
                               tep_draw_hidden_field('AcceptURL', tep_href_link(FILENAME_CHECKOUT_PROCESS, '', 'SSL')) .
                               tep_draw_hidden_field('CancelURL', tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL')) .
    ...
    function before_process() {
      global $customer_id, $order, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_webtopay_id;
      global $$payment;
      ...
      tep_db_query("update " . TABLE_ORDERS . " set orders_status = '" . (MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID > 0 ? (int)MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID : (int)DEFAULT_ORDERS_STATUS_ID) . "', last_modified = now() where orders_id = '" . (int)$order_id . "'");
...
// unregister session variables used during checkout
    ...
    function keys() {
      return array('MODULE_PAYMENT_WEBTOPAY_STATUS', 'MODULE_PAYMENT_WEBTOPAY_ID', 'MODULE_PAYMENT_WEBTOPAY_SORT_ORDER', 'MODULE_PAYMENT_WEBTOPAY_ZONE', 'MODULE_PAYMENT_WEBTOPAY_PREPARE_ORDER_STATUS_ID', 'MODULE_PAYMENT_WEBTOPAY_ORDER_STATUS_ID');
    }
