<?php
/*
  $Id: easypay.php 1803 2008-01-11 18:16:37Z hpdl $
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_EASYPAY_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_EASYPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function selection() {
      global $cart_easypay_id;

      if (tep_session_is_registered('cart_easypay_id')) {
        $order_id = substr($cart_easypay_id, strpos($cart_easypay_id, '-')+1);
        ...
        if (tep_db_num_rows($check_query) < 1) {
          ...
          tep_session_unregister('cart_easypay_id');
        }
      }
    ...
    function confirmation() {
      global $cartID, $cart_easypay_id, $customer_id, $languages_id, $order, $order_total_modules;
      ...
      if (tep_session_is_registered('cartID')) {
        ...
        if (tep_session_is_registered('cart_easypay_id')) {
          ...
          if ( ($curr['currency'] != $order->info['currency']) || ($cartID != substr($cart_easypay_id, 0, strlen($cartID))) ) {
            $check_query = tep_db_query('select orders_id from ' . TABLE_ORDERS_STATUS_HISTORY . ' where orders_id = "' . (int)$order_id . '" limit 1');
...
//PIN
          ...
          $cart_easypay_id = $cartID . '-' . $insert_id;
          tep_session_register('cart_easypay_id');
        }
      }
    ...
    function process_button() {
      global $customer_id, $order, $sendto, $currency, $currencies, $cart_easypay_id, $shipping;
    ...
    function before_process() {
      global $customer_id, $order, $order_totals, $sendto, $billto, $languages_id, $payment, $currencies, $cart, $cart_easypay_id;
      global $$payment;
...
// unregister session variables used during checkout
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Разрешить модуль оплаты EasyPay', 'MODULE_PAYMENT_EASYPAY_STATUS', 'True', 'Разрешить модуль оплаты EasyPay?', '6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('EasyPay ID', 'MODULE_PAYMENT_EASYPAY_MERCHNO', '', 'Номер Поставщика в EasyPay. Выдается Администратором при подключении к EasyPay.', '6', '4', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Порядок сортировки.', 'MODULE_PAYMENT_EASYPAY_SORT_ORDER', '0', 'Порядок сортировки модуля.', '6', '6', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Зона оплаты', 'MODULE_PAYMENT_EASYPAY_ZONE', '0', 'Если выбрана зона, данный модуль оплаты будет доступен только покупателям из указанной зоны.', '6', '7', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Электронный ключ', 'MODULE_PAYMENT_EASYPAY_WEBKEY', '0', 'Электронный ключ. Выдается Администратором при подключении к EasyPay', '6', '8', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Отладочный режим', 'MODULE_PAYMENT_EASYPAY_DEBUG', '0', 'Режим отладки (0 или отсутствует – отладочный режим отключен, 1 – включен)', '6', '9', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Укажите оплаченный статус заказа', 'MODULE_PAYMENT_EASYPAY_ORDER_STATUS_ID', '0', 'Укажите оплаченный статус заказа', '6', '9', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
    }
    ...
    function keys() {
      return array('MODULE_PAYMENT_EASYPAY_STATUS', 'MODULE_PAYMENT_EASYPAY_MERCHNO', 'MODULE_PAYMENT_EASYPAY_SORT_ORDER', 'MODULE_PAYMENT_EASYPAY_ZONE', 'MODULE_PAYMENT_EASYPAY_WEBKEY', 'MODULE_PAYMENT_EASYPAY_DEBUG','MODULE_PAYMENT_EASYPAY_ORDER_STATUS_ID');
    }
