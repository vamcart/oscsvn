<?php
/*
  $Id: affiliate_banners.php,v 1.2 2003/09/24 13:57:05 wilt Exp $
...
*/
         if (empty($affiliate_banners_html_text)) {
          if (empty($affiliate_banners_image_local)) {
            $affiliate_banners_image = new upload('affiliate_banners_image');
            $affiliate_banners_image->set_destination(DIR_FS_CATALOG_IMAGES);
            if ($affiliate_banners_image->parse() == true) {
               if (tep_not_null($affiliate_banners_image_target))
                  $affiliate_banners_image->set_filename($affiliate_banners_image_target);
               if ($affiliate_banners_image->save() == false)
                 $affiliate_banner_error = true;
            } else {
               $affiliate_banner_error = true;
            }
        ...
        if (!$affiliate_banner_error) {
          $db_image_location = (!empty($affiliate_banners_image_local)) ? $affiliate_banners_image_local : $affiliate_banners_image->filename; 
          ...
          tep_redirect(tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $_GET['page'] . '&abID=' . $affiliate_banners_id));
        } else {
          $_GET['action'] = 'new';
        }
        break;
...
<?php
    ...
    $affiliate_banners_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $affiliate_banners_query_raw, $affiliate_banners_query_numrows);
...
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $affiliate_banners_split->display_count($affiliate_banners_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_BANNERS); ?></td>
                    <td class="smallText" align="right"><?php echo $affiliate_banners_split->display_links($affiliate_banners_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
                  <tr>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'delete':
      ...
      $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $_GET['page'] . '&abID=' . $_GET['abID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
      break;
    default:
      if (is_object($abInfo)) {
