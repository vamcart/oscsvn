<?php
/*
  $Id: reviews.php,v 1.2 2003/09/24 15:18:15 wilt Exp $
...
function reviews_set_status_otz($reviews_id, $sticky) {
if ($sticky == '1') {
	return tep_db_query("update " . TABLE_REVIEWS . " set status_otz = '1', last_modified = now() where reviews_id = '" . $reviews_id . "'");
} elseif ($sticky == '0') {
	return tep_db_query("update " . TABLE_REVIEWS . " set status_otz = '0', last_modified = now() where reviews_id = '" . $reviews_id . "'");
} else {
	return -1;
}
...
if ( ($_GET['flag'] == '0') || ($_GET['flag'] == '1') ) {
	if ($_GET['pID']) {
		reviews_set_status_otz($_GET['pID'], $_GET['flag']);
	}
}
}
}
...
<?php
    $reviews_query_raw = "select reviews_id, products_id, date_added, status_otz, last_modified, reviews_rating from " . TABLE_REVIEWS . " order by date_added DESC";
...
<?php
if ($reviews['status_otz'] == '1') {
	echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_REVIEWS, 'action=setflag&flag=0&pID=' . $reviews['reviews_id'] . '&cPath=' . $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
} else {
	echo '<a href="' . tep_href_link(FILENAME_REVIEWS, 'action=setflag&flag=1&pID=' . $reviews['reviews_id'] . '&cPath=' . $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
}
?>
                </td>
                
                <td class="dataTableContent" align="right"><?php if ( (is_object($rInfo)) && ($reviews['reviews_id'] == $rInfo->reviews_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif'); } else { echo '<a href="' . tep_href_link(FILENAME_REVIEWS, 'page=' . $_GET['page'] . '&rID=' . $reviews['reviews_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $reviews_split->display_count($reviews_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_REVIEWS); ?></td>
                    <td class="smallText" align="right"><?php echo $reviews_split->display_links($reviews_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
