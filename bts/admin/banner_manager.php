<?php
/*
  $Id: banner_manager.php,v 1.2 2003/09/24 13:57:05 wilt Exp $
    ...
    switch ($action) {
      ...
      case 'deleteconfirm':
        ...
        if (isset($_POST['delete_image']) && ($_POST['delete_image'] == 'on')) {
          ...
          if (is_file(DIR_FS_CATALOG_IMAGES . 'banner/' . $banner['banners_image'])) {
            if (is_writeable(DIR_FS_CATALOG_IMAGES . 'banner/' . $banner['banners_image'])) {
              unlink(DIR_FS_CATALOG_IMAGES . 'banner/' . $banner['banners_image']);
            } else {
      ...
      <tr><?php echo tep_draw_form('new_banner', FILENAME_BANNER_MANAGER, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'action=' . $form_action, 'post', 'enctype="multipart/form-data"'); if ($form_action == 'update') echo tep_draw_hidden_field('banners_id', $bID); ?>
        <td><table border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
            ...
            <td valign="top" class="main"><?php echo tep_draw_input_field('date_scheduled', $bInfo->date_scheduled, 'size="10" class="format-d-m-y dividor-slash"'); ?></td>
          </tr>
          <tr>
            <td colspan="2"><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?></td>
          </tr>
          <tr>
            ...
            <td class="main"><?php echo tep_draw_input_field('expires_date', $bInfo->expires_date, 'size="10" class="format-d-m-y dividor-slash"'); ?><?php echo TEXT_BANNERS_OR_AT . '<br>' . tep_draw_input_field('expires_impressions', $bInfo->expires_impressions, 'maxlength="7" size="7"') . ' ' . TEXT_BANNERS_IMPRESSIONS; ?></td>
          </tr>
        </table></td>
      </tr>
      <tr>
...
<?php
    ...
    $banners_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $banners_query_raw, $banners_query_numrows);
...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $banners_split->display_count($banners_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_BANNERS); ?></td>
                    <td class="smallText" align="right"><?php echo $banners_split->display_links($banners_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
                  <tr>
