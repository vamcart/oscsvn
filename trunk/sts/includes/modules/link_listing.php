    ...
    while ($listing = tep_db_fetch_array($listing_query)) {
      ...
      for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
        ...
        switch ($column_list[$col]) {
          case 'LINK_LIST_TITLE':
            $lc_align = '';
            $lc_text = '<a href="' . $listing['links_url'] . '" target="_blank">' . $listing['links_title'] . '</a>';
            break;
          case 'LINK_LIST_URL':
            $lc_align = '';
            $lc_text = '<a href="' . $listing['links_url'] . '" target="_blank">' . $listing['links_url'] . '</a>';
            break;
          case 'LINK_LIST_DESCRIPTION':
            $lc_align = '';
          ...
          case 'LINK_LIST_IMAGE':
            ...
            if (tep_not_null($listing['links_image_url'])) {
              $lc_text = '<a href="' . $listing['links_url'] . '" target="_blank">' . tep_links_image($listing['links_image_url'], $listing['links_title'], LINKS_IMAGE_WIDTH, LINKS_IMAGE_HEIGHT) . '</a>';
            } else {
              $lc_text = '<a href="' . $listing['links_url'] . '" target="_blank">' . tep_image(DIR_WS_IMAGES . 'pixel_trans.gif', $listing['links_title'], LINKS_IMAGE_WIDTH, LINKS_IMAGE_HEIGHT, 'style="border: 0px"') . '</a>';
            }
            break;
          case 'LINK_LIST_COUNT':
            $lc_align = '';
