...
//  $ShipTxt= tep_draw_form('estimator', tep_href_link(FILENAME_SHOPPING_CART, '', 'NONSSL'), 'post'); //'onSubmit="return check_form();"'
  ...
  if(sizeof($quotes)) {
    if (tep_session_is_registered('customer_id')) {
      ...
      $ShipTxt.='<tr><td colspan="3" class="main">' . ($total_count == 1 ? SHIPPING_METHOD_QTY : SHIPPING_METHOD_QTY) . $total_count . ',&nbsp;' . SHIPPING_METHOD_WEIGHT . $total_weight . SHIPPING_METHOD_WEIGHT1 . '</td></tr>';
