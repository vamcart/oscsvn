     ...
     function vam_RandomString($length) {
       ...
       for($i=0;$i<$length;$i++)
       {
         ...
         return $rand_str;
          ...
          $makePassword = vam_RandomString(8);
...
// BOF: KategorienAdmin / OLISWISS
          if ($_POST['admin_groups_id'] == 1) {
            ...
            $admin_right_access = implode(',',$_POST['admin_right_access']);
            $admin_cat_access = implode(',',$_POST['admin_cat_access']);
          }
// EOF: KategorienAdmin / OLISWISS
          ...
          $sql_data_array = array('admin_groups_id' => tep_db_prepare_input($_POST['admin_groups_id']),
                                  ...
                                  'admin_password' => tep_encrypt_password($_POST['admin_password']),
// BOF: KategorienAdmin / OLISWISS
...
// EOF: KategorienAdmin / OLISWISS
          ...
          tep_mail($_POST['admin_firstname'] . ' ' . $_POST['admin_lastname'], $_POST['admin_email_address'], ADMIN_EMAIL_SUBJECT, sprintf(ADMIN_EMAIL_TEXT, $_POST['admin_firstname'], HTTP_SERVER . DIR_WS_ADMIN, $_POST['admin_email_address'], $_POST['admin_password'], STORE_OWNER), STORE_OWNER, STORE_OWNER_EMAIL_ADDRESS);
...
<?php
  ...
  $db_admin_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $db_admin_query_raw, $db_admin_query_numrows);
...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $db_admin_split->display_count($db_admin_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_MEMBERS); ?><br><?php echo $db_admin_split->display_links($db_admin_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
...
<?php
  ...
  switch ($_GET['action']) {
    case 'new_member':
      ...
      $contents[] = array('text' => '<br>&nbsp;' . TEXT_INFO_PASSWORD . '<br>&nbsp;' . tep_draw_password_field('admin_password'));

// BOF: KategorienAdmin / OLISWISS
      $str_rights = $mInfo->admin_right_access;
      if (strstr($str_rights,"CNEW") || $str_rights == "") $cnew_checked = true; else $cnew_checked = false;
      if (strstr($str_rights,"CEDIT") || $str_rights == "") $cedit_checked = true; else $cedit_checked = false;
      if (strstr($str_rights,"CMOVE") || $str_rights == "") $cmove_checked = true; else $cmove_checked = false;
      if (strstr($str_rights,"CDELETE") || $str_rights == "") $cdelete_checked = true; else $cdelete_checked = false;
      if (strstr($str_rights,"PNEW") || $str_rights == "") $pnew_checked = true; else $pnew_checked = false;
      if (strstr($str_rights,"PEDIT") || $str_rights == "") $pedit_checked = true; else $pedit_checked = false;
      if (strstr($str_rights,"PMOVE") || $str_rights == "") $pmove_checked = true; else $pmove_checked = false;
      if (strstr($str_rights,"PCOPY") || $str_rights == "") $pcopy_checked = true; else $pcopy_checked = false;
      if (strstr($str_rights,"PDELETE") || $str_rights == "") $pdelete_checked = true; else $pdelete_checked = false;

        $all_categories .= tep_draw_checkbox_field('admin_right_access[CNEW]', 'CNEW',$cnew_checked) . " " . TEXT_RIGHTS_CNEW . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[CEDIT]', 'CEDIT',$cedit_checked) . " " . TEXT_RIGHTS_CEDIT . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[CMOVE]', 'CMOVE',$cmove_checked) . " " . TEXT_RIGHTS_CMOVE . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[CDELETE]', 'CDELETE',$cdelete_checked) . " " . TEXT_RIGHTS_CDELETE . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[PNEW]', 'PNEW',$pnew_checked) . " " . TEXT_RIGHTS_PNEW . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[PEDIT]', 'PEDIT',$pedit_checked) . " " . TEXT_RIGHTS_PEDIT . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[PMOVE]', 'PMOVE',$pmove_checked) . " " . TEXT_RIGHTS_PMOVE . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[PCOPY]', 'PCOPY',$pcopy_checked) . " " . TEXT_RIGHTS_PCOPY . "<br>&nbsp;";
        $all_categories .= tep_draw_checkbox_field('admin_right_access[PDELETE]', 'PDELETE',$pdelete_checked) . " " . TEXT_RIGHTS_PDELETE . "<br><br>&nbsp;";
        ...
        while ($n < tep_db_num_rows($top_categories_query)) {
          ...
          $top_categories_name = tep_db_fetch_array($top_categories_name_query);
			    $all_categories .= tep_draw_checkbox_field('admin_cat_access[' . $top_categories['categories_id'] . ']', $top_categories['categories_id'], (in_array($top_categories['categories_id'], $str_cat_no_array) || $mInfo->admin_cat_access == "ALL")) . " " . $top_categories_name['categories_name'] . " (" . TEXT_RIGHTS_ID ." " . $top_categories['categories_id'] . ")<br>&nbsp;";
			    $n = $n + 1;
  			}
