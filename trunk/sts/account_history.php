...
<?php
  ...
  if ($orders_total > 0) {
    $history_query_raw = "select o.orders_id, o.date_purchased, o.payment_method, o.delivery_name, o.billing_name, ot.text as order_total, s.orders_status_name from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, " . TABLE_ORDERS_STATUS . " s where o.customers_id = '" . (int)$customer_id . "' and o.orders_id = ot.orders_id and ot.class = 'ot_total' and o.orders_status = s.orders_status_id and s.language_id = '" . (int)$languages_id . "' order by orders_id DESC";
            ...
            <tr class="infoBoxContents">
              <td><table border="0" width="100%" cellspacing="2" cellpadding="4">
                <tr>
                  ...
                  <td class="main" width="20%"><?php echo '<a href="' . tep_href_link(FILENAME_ACCOUNT_HISTORY_INFO, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'order_id=' . $history['orders_id'], 'SSL') . '">' . tep_image_button('small_view.gif', SMALL_IMAGE_BUTTON_VIEW) . '</a>'; ?>
                  
<?php
require(DIR_WS_LANGUAGES . $language . '/modules/payment/schet.php');
if ($history['payment_method'] == MODULE_PAYMENT_RUS_SCHET_TEXT_TITLE) {
?>
...
//--></script>
          ...
          <a href="javascript:popupPrintorder('<?php echo tep_href_link(FILENAME_ORDERS_PRINTABLE, 'order_id=' . $history['orders_id']); ?>')"><?php echo IMAGE_BUTTON_PRINT_ORDER; ?></a>         
...
//--></script>
          ...
          <a href="javascript:popupPrintorder('<?php echo tep_href_link('kvitan.php', 'order_id=' . $history['orders_id']); ?>')"><?php echo MODULE_PAYMENT_RUS_BANK_TEXT_PRINT; ?></a>
