    ...
    if ($listing_split->number_of_rows > 0) {
      ...
      while ($listing = tep_db_fetch_array($listing_query)) {
        ...
        if (PRODUCT_LIST_BUY_NOW) {
          $lc_text .= '';   	  
          $lc_text .= '<br><a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now&products_id=' . $listing['products_id'], 'NONSSL') . '">' . tep_template_image_button('button_buy_now.gif', TEXT_BUY . $listing['products_name'] . TEXT_NOW) . '</a>';   	  
        }
