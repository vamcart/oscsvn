<?php
/*
  $Id: specials.php,v 1.2 2003/09/24 15:18:15 wilt Exp $
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'deleteconfirm':
        ...
        $product_query = tep_db_query("select products_id from " . TABLE_SPECIALS . " where specials_id = '" . (int)$specials_id . "'");
        $product = tep_db_fetch_array($product_query);

        tep_db_query("delete from " . TABLE_SPECIAL_PRODUCT . " where product_id = '" . (int)$product['products_id'] . "'");
...
<?php
    ...
    $specials_split = new splitPageResults($_GET['page'], MAX_PROD_ADMIN_SIDE, $specials_query_raw, $specials_query_numrows);
...
?>
              <tr>
                <td colspan="4"><table border="0" width="100%" cellpadding="0"cellspacing="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $specials_split->display_count($specials_query_numrows, MAX_PROD_ADMIN_SIDE, $_GET['page'], TEXT_DISPLAY_NUMBER_OF_SPECIALS); ?></td>
                    <td class="smallText" align="right"><?php echo $specials_split->display_links($specials_query_numrows, MAX_PROD_ADMIN_SIDE, MAX_DISPLAY_PAGE_LINKS, $_GET['page']); ?></td>
                  </tr>
<?php
  if (empty($action)) {
?>
                  <tr>
                    <td colspan="2" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_SELECT_SPECIAL, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_image_button('button_search.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>&nbsp;<?php echo '<a href="' . tep_href_link(FILENAME_SPECIALS, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_image_button('button_new_product.gif', IMAGE_NEW_PRODUCT) . '</a>'; ?>&nbsp;<?php echo '<a href="' . tep_href_link(FILENAME_CATEGORY_SPECIALS, 'page=' . $_GET['page'] . '&action=new') . '">' . tep_image_button('button_new_category.gif', IMAGE_NEW_CATEGORY) . '</a>'; ?></td>
                  </tr>
<?php
  }
?>
