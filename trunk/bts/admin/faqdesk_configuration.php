...
$cID = tep_db_prepare_input($_GET['cID']);
          ...
          while ($configuration = tep_db_fetch_array($configuration_query))
              tep_db_query("UPDATE ".TABLE_FAQDESK_CONFIGURATION." SET configuration_value='".$_POST[$configuration['configuration_key']]."' where configuration_key='".$configuration['configuration_key']."'");
               ...
               tep_redirect(FILENAME_FAQDESK_CONFIGURATION. '?gID=' . (int)$_GET['gID']);
        break;
...
<?php echo tep_draw_form('configuration', FILENAME_FAQDESK_CONFIGURATION, 'gID=' . (int)$_GET['gID'] . '&action=save'); ?>
...
<?php
  ...
  while ($configuration = tep_db_fetch_array($configuration_query)) {
    ...
    if (((!$_GET['cID']) || (@$_GET['cID'] == $configuration['configuration_id'])) && (!$cInfo) && (substr($_GET['action'], 0, 3) != 'new')) {
      $cfg_extra_query = tep_db_query("select configuration_key,configuration_value, date_added, last_modified, use_function, set_function from " . TABLE_FAQDESK_CONFIGURATION . " where configuration_id = '" . $configuration['configuration_id'] . "'");
