<?php
/*
  $Id: create_order_details.php,v 1 2002/11/28 23:22:27 wilt Exp $
...
?>
          <tr>
            <td class="main">&nbsp;<?php echo TABLE_HEADING_CUSTOMERS_GROUP; ?></td>
            <td class="main">&nbsp;
<?php
if ($_GET['Customer'] != 0) {
	$customers_query_raw = "select customers_groups_id from " . TABLE_CUSTOMERS . "  where customers_id = " . $_GET['Customer'] . "";
    //TotalB2B end
    ...
    while ($customers = tep_db_fetch_array($customers_query)) {
      ...
      $info = tep_db_fetch_array($info_query);
			$cust_coll = tep_db_query ("select customers_groups_id, customers_groups_name, customers_groups_discount FROM ". TABLE_CUSTOMERS_GROUPS ." where customers_groups_id = ". (int)$customers['customers_groups_id'] ."");
			$cust_ret = tep_db_fetch_array($cust_coll);

}

echo $cust_ret['customers_groups_name'];

} else { echo 'Выберите покупателя'; }
