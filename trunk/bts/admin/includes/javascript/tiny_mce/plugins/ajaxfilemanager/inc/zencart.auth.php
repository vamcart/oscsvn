    ...
    // Modified piece of code from whos_online.php
        ...
        if (STORE_SESSIONS == 'mysqli') {
          $session_data_query = mysqli_query($zen_mysqli_link,"select value from " . DB_PREFIX . "sessions
                                        WHERE sesskey = '" . $info . "'", $zen_mysqli_link);
          ...
          if (!defined('SESSION_WRITE_DIRECTORY')) {
            $session_write_directory_query = mysqli_query($zen_mysqli_link,"SELECT * FROM " . DB_PREFIX . "configuration WHERE configuration_key='SESSION_WRITE_DIRECTORY'", $zen_mysqli_link);
...
/*
        if (isset($_SESSION['admin_id'])) {
          $admin_name_query = mysqli_query($zen_mysqli_link,"SELECT * FROM " . DB_PREFIX . "admin WHERE admin_id=" . (int)$_SESSION['admin_id'], $zen_mysqli_link);
