...
<div class="boxHeader">
<h5><?php echo BOX_HEADING_DOWNLOAD; ?></h5>
</div>
...
echo BOX_TEXT_DOWNLOAD . '<br><br><a href="' . tep_href_link(FILENAME_DOWNLOAD, 'order=' . $last_order . '&id=' . $downloads_values['orders_products_download_id']) . '">' . $downloads_values['products_name'] . '</a>&nbsp;-&nbsp;<a href="' . tep_href_link(FILENAME_DOWNLOAD, 'order=' . $last_order . '&id=' . $downloads_values['orders_products_download_id']) . '"><font color="red"><b>' . BOX_TEXT_DOWNLOAD_NOW . '</b></font></a>';
...
<!-- right box -->
<?php
 echo TABLE_HEADING_DOWNLOAD_DATE . '<br>' .  tep_date_long($download_expiry);

  echo $downloads_values['download_count'] . '  ' .  TABLE_HEADING_DOWNLOAD_COUNT;
 }

if (!strstr($PHP_SELF, FILENAME_ACCOUNT_HISTORY_INFO)) {

  echo TEXT_FOOTER_DOWNLOAD . '<br><a href="' . tep_href_link(FILENAME_ACCOUNT_HISTORY, '', 'SSL') . '">' . TEXT_DOWNLOAD_MY_ACCOUNT . '</a>';
