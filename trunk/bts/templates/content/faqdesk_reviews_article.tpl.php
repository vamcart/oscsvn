...
<?php
echo tep_image(DIR_WS_TEMPLATES . TEMPLATE_NAME . '/images/content/reviews.gif', sprintf(HEADING_TITLE, $product_info_values['faqdesk_question']), HEADING_IMAGE_WIDTH, HEADING_IMAGE_HEIGHT);
?>
		</td>
	</tr>
</table>
...
if (tep_db_num_rows($reviews)) {
	...
	while ($reviews_values = tep_db_fetch_array($reviews)) {
		...
		echo '<td align="center" class="smallText">' . tep_image(DIR_WS_TEMPLATES . TEMPLATE_NAME . '/images/content/stars_' . $reviews_values['reviews_rating'] . '.gif', sprintf(TEXT_OF_5_STARS, $reviews_values['reviews_rating'])) . '</td>' . "\n";
...
?>
	...
	<tr>
		<td class="main">
<?php
echo '<a href="' . tep_href_link(FILENAME_FAQDESK_INFO, $get_params_back, 'NONSSL') . '">' . tep_template_image_button('button_back.gif', IMAGE_BUTTON_BACK) . '</a>';
?>
		</td>
		<td align="right" class="main">
<?php 
echo '<a href="' . tep_href_link(FILENAME_FAQDESK_REVIEWS_WRITE, $get_params, 'NONSSL') . '">' . tep_template_image_button('button_write_review.gif', IMAGE_BUTTON_WRITE_REVIEW) . '</a>';
?>
		</td>
	</tr>
</table>
